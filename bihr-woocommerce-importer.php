...
if ( defined( 'WP_CLI' ) && WP_CLI ) {
    require_once BIHRWI_PLUGIN_DIR . 'includes/class-bihr-cli.php';
    WP_CLI::add_command( 'bihr', 'BihrWI_CLI_Commands' );
}
...
/**
 * Migre automatiquement les credentials stockés en clair vers le format chiffré.
 */
add_action( 'plugins_loaded', 'bihrwi_migrate_credentials', 5 );
function bihrwi_migrate_credentials() {
    $options = array( 'bihrwi_password', 'bihrwi_openai_key' );
    foreach ( $options as $option ) {
        $value = get_option( $option, '' );
        if ( '' !== $value && 0 !== strpos( $value, 'enc:' ) ) {
            update_option( $option, bihrwi_encrypt_credential( $value ) );
            if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                error_log( '[BIHR] Migration automatique du credential ' . $option . ' vers le format chiffré.' );
            }
        }
    }
}
