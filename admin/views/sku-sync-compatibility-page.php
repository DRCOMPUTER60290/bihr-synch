...
if ( ! current_user_can( 'manage_woocommerce' ) ) {
    wp_die( esc_html__( 'Accès refusé.', 'bihr-woocommerce-importer' ) );
}
...
?>
<div class="wrap">
    <h1><?php esc_html_e( 'Synchronisation SKU depuis Compatibilité Véhicules', 'bihr-woocommerce-importer' ); ?></h1>

    <?php
    if ( $action === 'sync' ) {
        if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), $nonce_action ) ) {
            echo '<div class="notice notice-error"><p>' . esc_html__( 'Nonce invalide. Rechargez la page et réessayez.', 'bihr-woocommerce-importer' ) . '</p></div>';
        } else {
            echo '<h2>' . esc_html__( 'Synchronisation en cours…', 'bihr-woocommerce-importer' ) . '</h2>';
            ...
            if ( $new_offset < $total ) {
                ...
                echo '<p class="notice notice-warning" style="display:inline-block;padding:8px 12px;">' . esc_html__( 'Rechargement automatique dans 1 seconde…', 'bihr-woocommerce-importer' ) . '</p>';
            } else {
                $final_sku = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_key = '_sku' AND meta_value != ''" );
                echo '<h2 style="color:#00a32a;">' . esc_html__( 'Synchronisation terminée !', 'bihr-woocommerce-importer' ) . '</h2>';
                echo '<p style="color:#00a32a;">' . esc_html__( 'Total SKU synchronisés :', 'bihr-woocommerce-importer' ) . ' ' . esc_html( number_format_i18n( $final_sku ) ) . '</p>';
                echo '<a class="button" href="' . esc_url( $base_url ) . '">' . esc_html__( 'Retour', 'bihr-woocommerce-importer' ) . '</a>';
            }
        }
    } else {
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:15px;margin:20px 0;">
            <div style="background:#f0f6fc;padding:15px;border-radius:5px;border-left:4px solid #2271b1;">
                ...
                <?php esc_html_e( 'Produits WC avec compatibilité', 'bihr-woocommerce-importer' ); ?>
            </div>
            <div style="background:#f0f6fc;padding:15px;border-radius:5px;border-left:4px solid #2271b1;">
                <strong style="display:block;font-size:24px;color:#2271b1;"><?php echo esc_html( number_format_i18n( $wc_products ) ); ?></strong>
                <?php esc_html_e( 'Total produits WooCommerce', 'bihr-woocommerce-importer' ); ?>
            </div>
            <div style="background:#f0f6fc;padding:15px;border-radius:5px;border-left:4px solid #2271b1;">
                <strong style="display:block;font-size:24px;color:#00a32a;"><?php echo esc_html( number_format_i18n( $wc_sku_count ) ); ?></strong>
                <?php esc_html_e( 'SKU actuels', 'bihr-woocommerce-importer' ); ?>
            </div>
        </div>

        <div style="background:#fff3cd;border-left:4px solid #ffc107;padding:15px;margin:20px 0;">
            <h3 style="margin-top:0;">⚠️ <?php esc_html_e( 'Important', 'bihr-woocommerce-importer' ); ?></h3>
            <p><?php esc_html_e( 'Cette page synchronise les SKU en utilisant le part_number de la table de compatibilité véhicules, pas le product_code BIHR.', 'bihr-woocommerce-importer' ); ?></p>
            <p><?php esc_html_e( "Le match se fait dans l'ordre : NewPartNumber → SKU actuel → Code BIHR.", 'bihr-woocommerce-importer' ); ?></p>
        </div>

        <p>
            <a class="button button-primary" href="<?php echo esc_url( $sync_url ); ?>" onclick="return confirm('<?php echo esc_js( sprintf( __( 'Lancer la synchronisation de %s produits ?', 'bihr-woocommerce-importer' ), number_format_i18n( $wc_with_compatibility ) ) ); ?>');">
                🚀 <?php esc_html_e( 'LANCER LA SYNCHRONISATION', 'bihr-woocommerce-importer' ); ?>
            </a>
        </p>

        <h3>📊 <?php esc_html_e( 'Aperçu (10 premiers produits)', 'bihr-woocommerce-importer' ); ?></h3>
        <?php
        ...
        foreach ( $samples as $row ) {
            ...
            if ( $part_number === '' ) {
                ...
                $status = '<span style="color:#00a32a;">✅ ' . esc_html__( 'OK', 'bihr-woocommerce-importer' ) . '</span>';
            } else {
                $status = '<span style="color:#dba617;">⚠️ ' . esc_html__( 'SKU différent', 'bihr-woocommerce-importer' ) . '</span>';
            }
