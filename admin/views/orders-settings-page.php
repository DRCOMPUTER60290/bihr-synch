...
<div class="wrap">
    <h1>⚙️ <?php esc_html_e( 'Paramètres de Synchronisation des Commandes', 'bihr-synchronisation' ); ?></h1>
    ...
    <div class="bihr-section">
        <h2>🔄 <?php esc_html_e( 'Configuration de la Synchronisation Automatique', 'bihr-synchronisation' ); ?></h2>
        <p>
            <?php esc_html_e( 'Lorsqu\'une commande est créée dans WooCommerce, le plugin peut automatiquement la transmettre à l\'API BIHR pour traitement.', 'bihr-synchronisation' ); ?>
        </p>
        ...
        <form method="post" action="">
            ...
            <div class="form-group">
                <label>
                    ...
                    <strong><?php esc_html_e( 'Synchronisation automatique', 'bihr-synchronisation' ); ?></strong>
                </label>
                <p style="margin-left: 24px; margin-top: 4px; color: #6b7280; font-size: 13px;">
                    <?php esc_html_e( 'Envoyer automatiquement les commandes WooCommerce à l\'API BIHR lors de leur création.', 'bihr-synchronisation' ); ?>
                </p>
            </div>
            ...
            <div class="form-group">
                <label>
                    ...
                    <strong><?php esc_html_e( 'Validation automatique', 'bihr-synchronisation' ); ?></strong>
                </label>
                <p style="margin-left: 24px; margin-top: 4px; color: #6b7280; font-size: 13px;">
                    <?php esc_html_e( 'Valider automatiquement les commandes côté BIHR sans intervention manuelle.', 'bihr-synchronisation' ); ?>
                </p>
            </div>
            ...
            <div class="form-group">
                <label>
                    ...
                    <strong><?php esc_html_e( 'Livraison gratuite hebdomadaire', 'bihr-synchronisation' ); ?></strong>
                </label>
                <p style="margin-left: 24px; margin-top: 4px; color: #6b7280; font-size: 13px;">
                    <?php esc_html_e( 'Bénéficiez de la livraison gratuite hebdomadaire selon les conditions BIHR.', 'bihr-synchronisation' ); ?>
                </p>
            </div>

            <div class="form-group">
                <label for="bihrwi_delivery_mode">
                    <strong><?php esc_html_e( 'Mode de livraison', 'bihr-synchronisation' ); ?></strong>
                </label>
                <select id="bihrwi_delivery_mode" name="bihrwi_delivery_mode">
                    <option value="Default" <?php selected( $delivery_mode, 'Default' ); ?>>
                        <?php esc_html_e( 'Par défaut (Default)', 'bihr-synchronisation' ); ?>
                    </option>
                    <option value="Express" <?php selected( $delivery_mode, 'Express' ); ?>>
                        <?php esc_html_e( 'Express', 'bihr-synchronisation' ); ?>
                    </option>
                    <option value="Standard" <?php selected( $delivery_mode, 'Standard' ); ?>>
                        <?php esc_html_e( 'Standard', 'bihr-synchronisation' ); ?>
                    </option>
                </select>
                <p style="color: #6b7280; font-size: 13px; margin-top: 4px;">
                    <?php esc_html_e( 'Sélectionnez le mode de livraison par défaut pour toutes les commandes.', 'bihr-synchronisation' ); ?>
                </p>
            </div>

            <p class="submit">
                <button type="submit" name="bihrwi_save_order_settings" class="button button-primary large">
                    💾 <?php esc_html_e( 'Enregistrer les paramètres', 'bihr-synchronisation' ); ?>
                </button>
            </p>
        </form>
    </div>
    ...
    <div class="bihr-section">
        <h2><?php esc_html_e( 'Order/Data (BIHR)', 'bihr-synchronisation' ); ?></h2>
        <p>
            <?php esc_html_e( 'Saisissez un ID de commande WooCommerce pour récupérer les informations BIHR via GET /api/v2.1/Order/Data (TicketId provenant de la commande).', 'bihr-synchronisation' ); ?>
        </p>
        <p style="margin: 0 0 10px;">
            <label for="bihrwi_order_data_order_id" style="display:inline-block; min-width: 160px;">
                <?php esc_html_e( 'ID commande WooCommerce', 'bihr-synchronisation' ); ?>
            </label>
            ...
            <button type="button" class="button" id="bihrwi_order_data_fetch_btn">
                <?php esc_html_e( 'Récupérer', 'bihr-synchronisation' ); ?>
            </button>
        </p>
        ...
        <pre id="bihrwi_order_data_manual_pre" style="margin:0; max-height: 380px; overflow:auto; white-space: pre-wrap; word-break: break-word; background:#f6f7f7; padding:10px; border:1px solid #dcdcde; border-radius:4px;"><?php
            echo esc_html__( 'Renseignez un ID de commande et cliquez sur “Récupérer”.', 'bihr-synchronisation' );
        ?></pre>
    </div>

    <div class="bihr-section">
        <h2><?php esc_html_e( 'Informations de Synchronisation', 'bihr-synchronisation' ); ?></h2>
        
        <h3><?php esc_html_e( 'Comment ça fonctionne ?', 'bihr-synchronisation' ); ?></h3>
        <ol>
            <li><?php esc_html_e( 'Un client passe une commande sur votre boutique WooCommerce', 'bihr-synchronisation' ); ?></li>
            <li><?php esc_html_e( 'Le plugin vérifie si la commande contient des produits BIHR (avec un code produit BIHR)', 'bihr-synchronisation' ); ?></li>
            <li><?php esc_html_e( 'La commande est automatiquement envoyée à l\'API BIHR avec les informations de livraison', 'bihr-synchronisation' ); ?></li>
            <li><?php esc_html_e( 'Une note est ajoutée à la commande WooCommerce avec l\'ID de commande BIHR', 'bihr-synchronisation' ); ?></li>
            <li><?php esc_html_e( 'Les logs détaillés sont disponibles dans la page "Logs"', 'bihr-synchronisation' ); ?></li>
        </ol>

        <h3><?php esc_html_e( 'Format de la commande envoyée à BIHR', 'bihr-synchronisation' ); ?></h3>
        <pre style="background: #f0f0f1; padding: 15px; border: 1px solid #c3c4c7; border-radius: 4px; overflow-x: auto;">
{
  ...
  "DropShippingAddress": {
    ...
    "Phone": "+33123456789"
        ...
        <h3><?php esc_html_e( 'Données stockées dans WooCommerce', 'bihr-synchronisation' ); ?></h3>
        <table class="widefat">
            <thead>
                <tr>
                    <th><?php esc_html_e( 'Meta Key', 'bihr-synchronisation' ); ?></th>
                    <th><?php esc_html_e( 'Description', 'bihr-synchronisation' ); ?></th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td><code>_bihr_sync_ticket_id</code></td>
                    <td><?php esc_html_e( 'Ticket ID unique pour tracer toutes les étapes', 'bihr-synchronisation' ); ?></td>
                </tr>
                <tr>
                    <td><code>_bihr_order_synced</code></td>
                    <td><?php esc_html_e( 'Indique si la commande a été synchronisée avec succès', 'bihr-synchronisation' ); ?></td>
                </tr>
                <tr>
                    <td><code>_bihr_order_id</code></td>
                    <td><?php esc_html_e( 'ID de la commande côté BIHR', 'bihr-synchronisation' ); ?></td>
                </tr>
                <tr>
                    <td><code>_bihr_sync_date</code></td>
                    <td><?php esc_html_e( 'Date et heure de synchronisation', 'bihr-synchronisation' ); ?></td>
                </tr>
                <tr>
                    <td><code>_bihr_order_sync_failed</code></td>
                    <td><?php esc_html_e( 'Indique si la synchronisation a échoué', 'bihr-synchronisation' ); ?></td>
                </tr>
                <tr>
                    <td><code>_bihr_sync_error</code></td>
                    <td><?php esc_html_e( 'Message d\'erreur en cas d\'échec', 'bihr-synchronisation' ); ?></td>
                </tr>
            </tbody>
        </table>

        <h3 style="margin-top: 20px;"><?php esc_html_e( 'Notes importantes', 'bihr-synchronisation' ); ?></h3>
        <ul>
